...
- **Armadilha da imagem do SQL Server em contêiner.** O `sqlservr` dimensiona o próprio consumo pela **memória do nó**, não pelo limite do cgroup: num nó de 8 GB ele tentava alocar muito além do `limits.memory: 1Gi` que o manifesto declarava, o kernel cortava a alocação e o processo **abortava**. O sintoma é o pod em loop de restart (`restartCount: 12`), com término `reason=Error exitCode=235` e, no log do container anterior, `Failed to execute SystemTaskEntryPoint: unable to allocate MsqlXactManager` junto de `Error: 17300`. A correção tem duas partes e as duas são necessárias (`k8s/sqlserver-deployment.yaml`): `MSSQL_MEMORY_LIMIT_MB=1536` faz o SQL Server **respeitar o cgroup** — o valor é interno ao banco e fica abaixo dos 2 GB do container, deixando folga para o resto do processo —, e o `limits.memory` subiu para os **2 GB mínimos recomendados pela imagem**, com `requests` iguais (`memory: 2Gi`, `cpu: 500m`/`1000m`) para o agendador escolher um nó com espaço. Só aumentar o limite **não** resolve: sem o `MSSQL_MEMORY_LIMIT_MB` a imagem voltaria a dimensionar pela memória do nó e o loop reapareceria num nó maior. Aplicada ao vivo no cluster, a correção devolveu o banco: **6 usuários** preservados no volume e login pelo gateway respondendo **`200`**.
